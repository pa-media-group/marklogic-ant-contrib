    ...
    private void installPipeline(List<ResourceFileSet> resources) {
        try {
            for (final ResourceFileSet resource : resources) {
                ...
                System.out.println(" -- ".concat(targetDatabase).concat(" -- "));
                ...
                while (it.hasNext()) {
                    ...
                    try {
                        ...
                        System.out.println("Failed to read pipeline file ".concat(f));
                        throw Throwables.propagate(e);
                    } catch (RequestException e) {
                        System.out.println("Failed to insert pipeline file ".concat(f));
                        throw Throwables.propagate(e);
                    }
                }
            }
        ...
        try {
            for (ResourceFileSet resource : resources) {
                ...
                if (permissions != null && permissions.length > 0) {
                    ...
                    for (Permission permission : permissions) {
                        ContentPermission contentPermission = new ContentPermission(
                                permission.getCapability(), permission.getRole());
                ...
                while (it.hasNext()) {
                    ...
                    System.out.println(
                            String.format("Deploying %s to %s", sourceFile.getPath(), destinationPath));
                    try {

                        Content c = newContent(destinationPath, getFileAsString(sourceFile), options);
                        session.insertContent(c);
                    } catch (IOException e) {
                        throw new BuildException("Failed to read content file ".concat(f), e);
                    } catch (RequestException e) {
                        throw new BuildException("Failed to insert content file ".concat(f), e);
                    }
                }
            }
        } finally {
